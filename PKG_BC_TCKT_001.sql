...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body PKG_BC_TCKT_001
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "KTNB_TEST"."PKG_BC_TCKT_001" AS 
   /*
    -- Mã/Tên ch?c n?ng: --TCKT_BC_03 – So chi tiet cac tai khoan
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
    ...
    BEGIN
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                'S? d? ??u k?:',
                dldk.ma_tk,
                decode(dldk.loai_tai_khoan, 1, dldk.so_tien, 3, decode(dldk.so_tien - abs(dldk.so_tien), 0, dldk.so_tien, 0))                                        so_du_dk_no,
                decode(dldk.loai_tai_khoan, 2, abs(dldk.so_tien), 3, decode(dldk.so_tien + abs(dldk.so_tien), 0, abs(dldk.so_tien)))                                 so_du_dk_co,
                '001',
                'B'
            FROM
                (
                    ...
                    FROM
                        tbd_tc_ht ht
                    WHERE
                            (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                        AND (p_ma_tk IS NOT NULL AND ht.ma_tk = p_ma_tk)
                        AND (p_ngay_tu IS NOT NULL AND ht.ngay_ht < p_ngay_tu)
                    GROUP BY
                        ht.ma_tk,
                        ht.loai_tai_khoan
                ) dldk;
        --Lay dieu chinh so du dau nam
        INSERT INTO tbr_tc_bc_001 (
            dien_giai,
            so_tien_vnd_no,
            so_tien_vnd_co,
            ...
            WHERE
                ...
                AND (p_ma_tk IS NOT NULL AND ht.ma_tk = p_ma_tk)
                AND ht.dc_sddn = 'X'
                AND ((p_ngay_tu IS NOT NULL AND ht.ngay_ht >= p_ngay_tu)  AND (p_ngay_den IS NOT NULL AND ht.ngay_ht < p_ngay_den+1))
           GROUP BY
                ht.ma_tk;        
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                '- C?ng phát sinh trong k?:'  dien_giai,
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            WHERE
                    (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                AND (p_ma_tk IS NOT NULL AND ht.ma_tk = p_ma_tk)
               AND ((p_ngay_tu IS NOT NULL AND ht.ngay_ht >= trunc(p_ngay_tu, 'YEAR')) AND (p_ngay_den IS NOT NULL AND ht.ngay_ht < p_ngay_den+1))
            GROUP BY
                ht.ma_tk;
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                ...
                dldk.ma_tk,
                decode(dldk.loai_tai_khoan, 1, dldk.so_tien, 3, decode(dldk.so_tien - abs(dldk.so_tien), 0, dldk.so_tien, 0))                                            so_du_dk_no,
                decode(dldk.loai_tai_khoan, 2, abs(dldk.so_tien), 3, decode(dldk.so_tien + abs(dldk.so_tien), 0, abs(dldk.so_tien),
                0))                 so_du_dk_co,
                '006',
                'B'
            FROM
                (
                    ...
                    WHERE
                            (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                AND (p_ma_tk IS NOT NULL AND ht.ma_tk = p_ma_tk)
               AND (p_ngay_den IS NOT NULL AND ht.ngay_ht < p_ngay_den+1)
                    GROUP BY
                        ht.ma_tk,
                        ht.loai_tai_khoan
                ) dldk;
    ...
    /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_04 - S? qu? ti?n m?t
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
    ...
    /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_05 - S? ti?n g?i ngân hàng, kho b?c
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 18/06/2021
    */
    ...
    BEGIN
        SELECT
            TRIM(TRAILING '0' FROM nvl(p_ma_tk, '00'))
        INTO p_ma_tk_cat
        FROM
            dual;
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            WHERE
                 (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                AND REGEXP_LIKE ( ht.ma_tk, p_ma_tk_cat )
                AND (p_so_tk_id IS NOT NULL AND ht.tkdt_nh_id = p_so_tk_id)
               AND (p_ngay_tu IS NOT NULL AND ht.ngay_ht < p_ngay_tu)
            GROUP BY
                ht.ma_tk,
                ht.loai_tai_khoan;
    ...
    END;
     ...
     /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_06 – S? chi ti?t qu? ti?n m?t và TGNH b?ng ngo?i t?
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
     ...
     --Lay so du dau ky
        INSERT INTO tbr_tc_bc_001 (
            ...
            WHERE
                    (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                AND (p_ma_tk IS NOT NULL AND ht.ma_tk = p_ma_tk)
                AND (p_loai_tien IS NOT NULL AND ht.loai_tien = p_loai_tien)
                AND (p_ngay_tu IS NOT NULL AND ht.ngay_ht < p_ngay_tu)
            GROUP BY
                ht.ma_tk,
                ht.loai_tai_khoan;
     --Lay dieu chinh so du
     INSERT INTO tbr_tc_bc_001 (
            dien_giai,
            st_no,
            so_tien_vnd_no,
            ...
            SELECT
                '?i?u ch?nh s? d? ??u k?:'  dien_giai,
                nvl(SUM(nvl(ht.st_no, 0)), 0),
                nvl(SUM(nvl(ht.so_tien_vnd_no, 0)), 0),
                nvl(SUM(nvl(ht.st_co, 0)), 0),
                nvl(SUM(nvl(ht.so_tien_vnd_co, 0)), 0),
                '002'                       thuoc_tinh_1,
                'B'
            FROM
                tbd_tc_ht ht
    ...
    /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_15 – S? chi ti?t các kho?n t?m thu
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
    ...
    --Lay dieu chinh so du dau ky
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                '?i?u ch?nh s? d? ??u k?:'  dien_giai,
                nvl(SUM(nvl(tt.thuoc_tinh_11, 0)), 0),
                nvl(SUM(nvl(tt.thuoc_tinh_12, 0)), 0),
                nvl(SUM(nvl(tt.thuoc_tinh_13, 0)), 0),
                nvl(SUM(nvl(tt.thuoc_tinh_14, 0)), 0),
                nvl(SUM(nvl(tt.thuoc_tinh_15, 0)), 0),
                nvl(SUM(nvl(tt.thuoc_tinh_16, 0)), 0),
                '002'                       thuoc_tinh_1,
                'B'                         thuoc_tinh_2
            FROM (SELECT   
                SUM(ht.so_tien_vnd_co)    thuoc_tinh_11,
                CASE
                ...
                END                  thuoc_tinh_15,
                CASE
                    WHEN NOT REGEXP_LIKE ( ht.ma_tk_doi_ung,
    ...
    /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_16 – S? chi ti?t chi ho?t ??ng
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
     ...
     --Lay so du dau ky
        INSERT INTO tbr_tc_bc_001 (
            ...
            WHERE
                    ...
                    (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                AND ht.ma_tk LIKE '611%'
                AND ( p_nguon_kinh_phi IS NOT NULL
                      AND p_nguon_kinh_phi = ht.ma_nguon_kp )
                AND ( p_loai_kinh_phi IS NOT NULL
                      AND p_loai_kinh_phi = ht.ma_loai_kp )
                AND ( p_khoan IS NOT NULL
                      AND p_khoan = ht.khoan )
                AND ( p_muc_tieu_muc IS NOT NULL
                      AND p_muc_tieu_muc = ht.mtm_id )
                AND (p_ngay_tu IS NOT NULL AND ht.ngay_ht < p_ngay_tu);
    --Lay dieu chinh so du dau ky
        INSERT INTO tbr_tc_bc_001 (
            dien_giai,
  ...
  /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_17 – S? chi phí s?n xu?t, kinh doanh (Ho?c ??u t? XDCB)
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 17/06/2021
    */
 ...
 /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_18 - S? chi ti?t chi phí
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 22/06/2021
    */
    ...
    END;
     ...
     /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_32: B?ng ??i chi?u s? d? tài kho?n ti?n g?i
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 28/06/2021
    */
    ...
    BEGIN
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            WHERE
                    ht.bt_id = bt.id
                AND (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT DISTINCT
                'Tài kho?n: ' || kbnh.so_tai_khoan     dien_giai,
                dl.tkdt_nh_id,
                '001'                                  thuoc_tinh_1,
                'B'                                    thuoc_tinh_2
            FROM
            ...
            WHERE
                dl.tkdt_nh_id = kbnh.id;
    END;

     /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_48 – Báo cáo B?ng cân ??i s? phát sinh
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 14/07/2021
    */
    ...
    BEGIN
    --Lay du lieu tai khoan tru 131x,331x,138x,338x
        INSERT INTO tbr_tc_bc_001 (
            ma_tk_id,
            ma_tk,
            ten_tk,
            ...
            SELECT
                ...
                SUM(dl.dcsddn_no)          dcsddn_no,
                SUM(dl.dcsddn_co)          dcsddn_co,
            ...
            --Lay so du dau nam
                    SELECT
                        ...
                        dldk.cap_tai_khoan,
                        dldk.tk_cap_tren_id,
                        decode(dldk.loai_tai_khoan, 1, dldk.so_tien, 3, decode(dldk.so_tien - abs(dldk.so_tien), 0, dldk.so_tien))                                           so_du_dn_no,
                        decode(dldk.loai_tai_khoan, 2, abs(dldk.so_tien), 3, decode(dldk.so_tien + abs(dldk.so_tien), 0, abs(dldk.so_tien)))       so_du_dn_co,
                        0 dcsddn_no,
                        0 dcsddn_co,
                        0                                                                                                                                                    so_tien_vnd_no,
                        0                                                                                                                                                    so_tien_vnd_co,
                    ...
                    FROM
                        (
                            ...
                            WHERE
                                    ht.ma_tk = dmtk.ma
                                AND (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                                AND substr(ht.ma_tk, 1, 3) NOT IN ( '131', '331','138','338') 
                                AND (p_nam IS NOT NULL AND ht.ngay_ht < trunc(p_nam, 'YEAR'))
                            GROUP BY
                                dmtk.id,
                                ht.ma_tk,
                                dmtk.ten,
                ...
                --Lay dieu chinh so du dau nam
                    SELECT
                        ...
                        0                                        so_du_dn_no,
                        0                                        so_du_dn_co,
                        SUM(nvl(ht.so_tien_vnd_no, 0)) dcsddn_no,
                        SUM(nvl(ht.so_tien_vnd_co, 0)) dcsddn_co,
                        0           so_tien_vnd_no,
                        0           so_tien_vnd_co,
                        0                                        so_du_cn_no,
                        0                                        so_du_cn_co
                    ...
                    WHERE
                        ...
                        AND ht.ma_tk = dmtk.ma
                         AND ht.dc_sddn = 'X'
                        AND trunc(ht.ngay_ht, 'DD') BETWEEN trunc(nvl(p_nam, TO_DATE('1900', 'YYYY')), 'YEAR') AND add_months(trunc(
                ...
                --Lay so phat sinh trong nam
                    ...
                    WHERE
                        ...
                        AND ht.ma_tk = dmtk.ma
                        AND ht.dc_sddn IS NULL
                        AND (p_nam IS NOT NULL AND ht.ngay_ht >= trunc(p_nam, 'YEAR') AND ht.ngay_ht < trunc((p_nam+1), 'YEAR'))
                    GROUP BY
                        dmtk.id,
                        ht.ma_tk,
                        dmtk.ten,
                ...
                --Lay so du cuoi nam
                    SELECT
                        ...
                        0                                                                                                                                                    so_tien_vnd_no,
                        0                                                                                                                                                    so_tien_vnd_co,
                        decode(dlcn.loai_tai_khoan, 1, dlcn.so_tien, 3, decode(dlcn.so_tien - abs(dlcn.so_tien), 0, dlcn.so_tien))                                           so_du_cn_no,
                        decode(dlcn.loai_tai_khoan, 2, abs(dlcn.so_tien), 3, decode(dlcn.so_tien + abs(dlcn.so_tien), 0, abs(dlcn.
                        so_tien)))       so_du_cn_co
                    FROM
                        (
                            SELECT
                            ...
                            WHERE
                                ...
                                AND substr(ht.ma_tk, 1, 3) NOT IN ( '131', '331','138','338')
                                AND ht.ma_tk = dmtk.ma
                               AND (p_nam IS NOT NULL AND ht.ngay_ht < trunc((p_nam+1), 'YEAR'))
                            GROUP BY
                                dmtk.id,
                                ht.ma_tk,
                                dmtk.ten,
    ...
    --Lay du lieu cho tai khoan 131x, 331x,138x,338x
        INSERT INTO tbr_tc_bc_001 (
            ma_tk_id,
            ma_tk,
            ten_tk,
                        ...
                        (
                            ...
                            FROM
                                (
                                    ...
                                    WHERE
                                             (p_ma_dvhq IS NOT NULL AND ht.MA_DVHQ = p_ma_dvhq)
                                        AND substr(ht.ma_tk, 1, 3) IN ( '131', '331','138','338' )
                                        AND (p_nam IS NOT NULL AND ht.ngay_ht < trunc(p_nam, 'YEAR'))
                                    GROUP BY
                                        ht.ma_tk,
                                        ht.dttk
    ...
    --Tinh tong
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                ...
                SUM(nvl(dl.dcsddn_no, 0))              dcsddn_no,
                SUM(nvl(dl.dcsddn_co, 0))              dcsddn_co,
    ...
    END;
      ...
      /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_30 - Báo cáo Báo cáo tình hình th?c hi?n d? toán KPTX c?a các nhi?m v? ???c chuy?n sang n?m sau (Không ph?i xét chuy?n)
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 20/07/2021
    */
    ...
    --LAY CHI TIEU 1
                ...
                (
                    ...
                    WHERE
                        ...
                        AND ht.ma_loai_nv IN ( '09', '10', '11', '12', '13',
                                               '14', '18', '19', '20' )
                        AND (p_nam IS NOT NULL AND ht.ngay_ht >= trunc(TO_DATE(p_nam,'YYYY'), 'YEAR') AND ht.ngay_ht < trunc(TO_DATE(p_nam+1,'YYYY'), 'YEAR'))             
                    GROUP BY
                        ht.ma_nguon_ns,
                        ht.khoan,
                        ht.ma_loai_nv
                    ...
                    WHERE
                        ...
                        AND ht.ma_loai_nv IN ( '09', '10', '11', '12', '13',
                                               '14', '17', '18', '19', '20' )
                        AND (p_nam IS NOT NULL AND ht.ngay_ht >= trunc(TO_DATE(p_nam-1,'YYYY'), 'YEAR') AND ht.ngay_ht < trunc(TO_DATE(p_nam,'YYYY'), 'YEAR'))             
                    GROUP BY
                        ht.ma_nguon_ns,
                        ht.khoan,
                        ht.ma_loai_nv
                    ...
                    FROM
                        (
                            ...
                            FROM
                                tbd_tc_tttu_ct    tttuct,
                                tbd_tc_tttu       tttu,
                                tbd_tc_dvhq_kbnh  kbnh
                            WHERE
                                    tttu.id = tttuct.tttu_id
                                ...
                                AND (p_nam IS NOT NULL AND tttuct.ngay_ht >= trunc(TO_DATE(p_nam,'YYYY'), 'YEAR') AND tttuct.ngay_ht < trunc(TO_DATE(p_nam+1,'YYYY'), 'YEAR'))
                            GROUP BY
                                tttuct.ma_nguon_ns,
                                tttuct.khoan
                        ) dl
        ...
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                ...
                SUM(nvl(dl.thuoc_tinh_15, 0)),
                SUM(nvl(dl.thuoc_tinh_16, 0)),
                SUM(nvl(dl.thuoc_tinh_12, 0)) + SUM(nvl(dl.thuoc_tinh_13, 0)) + SUM(nvl(dl.thuoc_tinh_14, 0)) - SUM(nvl(dl.thuoc_tinh_15,
    ...
    --Them ten chi tieu 2   
        INSERT INTO tbr_tc_bc_001 (
            ...
            SELECT
                2,
                'Kinh phí ch??ng trình MTQG và Ch??ng trình m?c tiêu (chi t?t t?ng ch??ng trình)',
                '003',
                'B'
            FROM
                dual;
    ...
    END;
      ...
      /*
    -- Mã/Tên ch?c n?ng: TCKT_BC_71 – Biên b?n ??i chi?u công n?
    -- Ng??i s?a: HoangV
    -- Ngày s?a: 22/08/2021
    */
                        ...
                        (
                            SELECT
                                ...
                                nvl(hddact.so_thanh_toan_luy_ke, 0) + (
                                    SELECT
                                        nvl(SUM(ht.so_tien_vnd_no), 0)
                                    FROM
                                        tbd_tc_ht ht
                                    WHERE
                                            ht.hddact_id = hddact.id      
                                        AND ht.ma_tk LIKE '331%'
                                        AND (p_ngay_den IS NOT NULL AND ht.ngay_ht < p_ngay_den+1)
                                )                                                           so_da_thanh_toan
                            FROM
                                tbd_tc_hddact hddact
                            WHERE
                                    (p_ma_dvhq IS NOT NULL AND hddact.MA_DVHQ = p_ma_dvhq)
                                AND (p_doi_tuong IS NOT NULL AND don_vi_thuc_hien = p_doi_tuong)
                                AND hddact.trang_thai = 1
                        ) dl
                ) loc
            WHERE
                loc.so_con_phai_thanh_toan != 0;
    END;
END PKG_BC_TCKT_001;
